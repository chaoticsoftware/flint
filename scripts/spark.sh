    ...
    case "$PLATFORM" in
        macos)
            if have brew; then
                ...
                fail "Homebrew is required to update GitHub CLI on macOS."
                exit 1
            fi
            ;;
        linux)
            case "$LINUX_PM" in
                apt)
                    ...
                    ;;
                dnf|yum)
                    sudo "$LINUX_PM" upgrade -y gh || sudo "$LINUX_PM" install -y gh
                    ;;
                pacman)
...
if have gh; then
    ...
    update_gh_cli
    ok "GitHub CLI is up to date: $(gh --version | head -n1)"
else
...
echo "    2. Node.js LTS"
echo "    3. Agency CLI"
echo "    4. GitHub CLI (gh) with dual-identity SSH keys and auth"
printf '%s━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━%s\n' "$C_OK" "$C_OFF"
